...
final class WatchComplicationCaptureTests: XCTestCase {
    ...
    @MainActor
    func testCaptureWatchComplicationCornerPNGWhenRequested() throws {
        let env = ProcessInfo.processInfo.environment
        let sentinelPath = "/tmp/rt-audit/CAPTURE_WATCH_COMPLICATION_CORNER"
        let sentinelOK = FileManager.default.fileExists(atPath: sentinelPath)
            || env["CAPTURE_WATCH_COMPLICATION_CORNER"] == "1"
        guard sentinelOK else {
            throw XCTSkip("Drop \(sentinelPath) or set CAPTURE_WATCH_COMPLICATION_CORNER=1 to render audit PNG")
        }

        let path = env["WATCH_COMPLICATION_CORNER_PNG_PATH"] ?? "/tmp/rt-audit/verify-watch-complication-corner.png"

        let view = WatchComplicationCornerChrome(
            gymScore: 82,
            workScore: 75,
            sleepScore: 80,
            readinessScore: 79
        ...
        let attachment = XCTAttachment(image: image)
        attachment.name = "verify-watch-complication-corner.png"
        attachment.lifetime = .keepAlways
        add(attachment)
    }
}
