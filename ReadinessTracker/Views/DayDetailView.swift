...
struct DayDetailView: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: RTLayout.sectionSpacing) {
                ...
                // Honest #341: RecoveryTrajectoryView on SpO2 (Sleep #269 / RHR #309 dual; ≥5 + strain).
                if spo2SeriesThroughDay.count >= 5 {
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailSpO2RecoveryTrajectory)
                    .slideIn(delay: 0.2445)
                }
                ...
                // Honest #339: MetricCorrelationView HRV↔Strain (extends #270/#310/#324/#325).
                if historyThroughDay.count >= 3 {
                    MetricCorrelationView(
                        history: historyThroughDay,
                        xMetric: .hrv,
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailMetricCorrelationHRVStrain)
                    .slideIn(delay: 0.2468)
                }

                // Honest #340: MetricCorrelationView RHR↔Strain (extends #339; completes Strain correlation pair).
                if historyThroughDay.count >= 3 {
                    MetricCorrelationView(
                        history: historyThroughDay,
                        xMetric: .restingHR,
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailMetricCorrelationRHRStrain)
                ...
                // Honest #343: MetricCorrelationView HRV↔SpO2 (extends #342 Sleep↔SpO2; SpO2 correlation cluster).
                if historyThroughDay.count >= 3 {
                    MetricCorrelationView(
                        history: historyThroughDay,
                        xMetric: .hrv,
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailMetricCorrelationHRVSpo2)
                    .slideIn(delay: 0.24705)
                }

                // Honest #344: MetricCorrelationView RHR↔SpO2 (extends #343 HRV↔SpO2; SpO2 correlation cluster).
                if historyThroughDay.count >= 3 {
                    MetricCorrelationView(
                        history: historyThroughDay,
                        xMetric: .restingHR,
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailMetricCorrelationRHRSpo2)
                    .slideIn(delay: 0.24706)
                }

                // Honest #345: MetricCorrelationView Strain↔SpO2 (extends #344 RHR↔SpO2; completes SpO2 correlation cluster).
                if historyThroughDay.count >= 3 {
                    MetricCorrelationView(
                        history: historyThroughDay,
                    ...
                    .accessibilityElement(children: .contain)
                    .accessibilityIdentifier(SurfaceID.dayDetailMetricCorrelationStrainSpo2)
                    .slideIn(delay: 0.24707)
                }
    ...
    // MARK: - SpO2 Momentum (Honest #337)
    private var dayDetailSpO2MomentumPoints: [(date: Date, mom: Double)] {
        spo2AnalyzedThroughDay.compactMap { point in
    ...
    private var dayDetailSpO2LatestMomentumBand: (label: String, color: Color) {
        guard let mom = dayDetailSpO2MomentumPoints.last?.mom else {
            return ("—", RTColor.secondaryText)
        }
        // SpO2/bloodOxygen: higherIsBetter — rising momentum is improving.
        let improving = mom > 0
    ...
    private var dayDetailSpO2MomentumYDomain: ClosedRange<Double> {
        let vals = dayDetailSpO2MomentumPoints.map(\.mom)
    ...
    private var dayDetailSpO2MomentumStrip: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("7-Day SpO2 Momentum")
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(RTColor.secondaryText)
                Spacer()
                if let mom = dayDetailSpO2MomentumPoints.last?.mom {
                    let sign = mom >= 0 ? "+" : ""
                    Text(String(format: "%@%.0f%% · %@", sign, mom * 100, dayDetailSpO2LatestMomentumBand.label))
                        .font(.caption2.weight(.semibold))
                        .foregroundStyle(dayDetailSpO2LatestMomentumBand.color)
                        .monospacedDigit()
                }
            }

            if !dayDetailSpO2MomentumPoints.isEmpty {
                Chart {
                    RuleMark(y: .value("Zero", 0))
                        .foregroundStyle(RTColor.tertiaryText.opacity(0.6))
                        .lineStyle(StrokeStyle(lineWidth: 1, dash: [3, 3]))

                    ForEach(Array(dayDetailSpO2MomentumPoints.enumerated()), id: \.offset) { _, point in
                        AreaMark(
                            x: .value("Date", point.date, unit: .day),
                            yStart: .value("Zero", 0),
                ...
                .frame(height: 72)
                .chartYScale(domain: dayDetailSpO2MomentumYDomain)
                .chartXAxis(.hidden)
                .chartYAxis {
                    AxisMarks(position: .leading, values: [dayDetailSpO2MomentumYDomain.lowerBound, 0, dayDetailSpO2MomentumYDomain.upperBound]) { value in
                        AxisGridLine().foregroundStyle(RTColor.divider)
                        AxisValueLabel {
                            if let v = value.as(Double.self) {
                                Text("\(Int(v * 100))%")
                ...
                Text("Need ≥8 days for SpO2 momentum")
                    .font(.caption)
                    .foregroundStyle(RTColor.secondaryText)
                    .frame(maxWidth: .infinity, alignment: .center)
                    .frame(height: 72)
