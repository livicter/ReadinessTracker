...
/// Elevates `nutrition.potassiumMg` (HK `dietaryPotassium`) into Tonight | Baseline
/// on Today Body after Dietary Sodium. Net-new NutritionSummary + HK (#191).
/// Prefer dietaryPotassium — next micronutrient leftover after sodium. Higher is better.
struct DietaryPotassiumTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, potassiumMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if potassiumMg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher potassium is generally favorable (soft ~3400 mg goal).
    ...
    private var deltaCaption: String {
        guard potassiumMg != nil else { return "No potassium logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text("\(Int(tonight.rounded()))")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(Int(tonight.rounded())) milligrams potassium, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(potassiumColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [potassiumColor.opacity(0.2), potassiumColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DietaryPotassiumBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.potassiumMg }
            .filter { $0 >= 0 }
