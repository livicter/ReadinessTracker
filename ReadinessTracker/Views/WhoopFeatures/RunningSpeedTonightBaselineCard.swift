...
/// Elevates HealthKit `runningSpeed` (m/s) into a Tonight | Baseline dual on
/// Today body after Running Power. Net-new DailyHealthData + HK (#160).
/// Finish run-intensity pair with runningPower (#159); unused sparse run pace HK.
struct RunningSpeedTonightBaselineCard: View {
    ...
    private var status: (label: String, color: Color) {
        ...
        // Recreational running pace (avg m/s).
        if t >= 3.3 { return ("Brisk", RTColor.optimal) }
        if t >= 2.8 { return ("Steady", RTColor.good) }
        if t >= 2.2 { return ("Easy", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No running speed logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.2f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Running speed \(String(format: "%.2f", $0)) meters per second, \(status.label)" }
                                ?? "No running speed"
                        )
                }
                ...
                .accessibilityIdentifier(SurfaceID.runningSpeedBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Running Speed")
                        ...
                        AnimatedSparkline(data: sparklineValues, color: gaitColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.runningSpeedSpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    ...
                    .frame(height: 88)
                    .accessibilityLabel("Running speed trend last seven days")
                }
            }
            .padding(16)
        }
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = chartPoints.map(\.value) + [base, tonight ?? base, 2.8].filter { $0 > 0 }
        let lo = max(1.5, (vals.min() ?? 2.2) - 0.2)
        let hi = max(lo + 0.8, (vals.max() ?? 3.5) + 0.2)
        return lo...hi
    }
