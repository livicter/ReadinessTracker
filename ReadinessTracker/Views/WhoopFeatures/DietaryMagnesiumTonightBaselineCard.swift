...
/// Elevates `nutrition.magnesiumMg` (HK `dietaryMagnesium`) into Tonight | Baseline
/// on Today Body after Dietary Calcium. Net-new NutritionSummary + HK (#199).
/// Prefer dietaryMagnesium — strong mineral Today UX after calcium/iron. Higher is better (soft 400 mg).
struct DietaryMagnesiumTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, magnesiumMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if magnesiumMg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher magnesium is generally favorable (soft ~400 mg goal).
    ...
    private var deltaCaption: String {
        guard magnesiumMg != nil else { return "No dietary magnesium logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMg(tonight)) milligrams dietary magnesium, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(magnesiumColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [magnesiumColor.opacity(0.2), magnesiumColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DietaryMagnesiumBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.magnesiumMg }
            .filter { $0 >= 0 }
