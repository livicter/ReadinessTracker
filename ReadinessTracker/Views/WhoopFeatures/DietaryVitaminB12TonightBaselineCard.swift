...
/// Elevates `nutrition.vitaminB12Mcg` (HK `dietaryVitaminB12`) into Tonight | Baseline
/// on Today Body after Vitamin D. Net-new NutritionSummary + HK (#196).
/// Prefer dietaryVitaminB12 — unused vitamin leftover after vitamin D. Higher is better (soft 2.4 mcg).
struct DietaryVitaminB12TonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, vitaminB12Mcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if vitaminB12Mcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher vitaminB12 is generally favorable (soft ~2.4 mcg goal).
    ...
    private var deltaCaption: String {
        guard vitaminB12Mcg != nil else { return "No vitamin B12 logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms vitamin B12, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(vitaminB12Color)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [vitaminB12Color.opacity(0.2), vitaminB12Color.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DietaryVitaminB12Baseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.vitaminB12Mcg }
            .filter { $0 >= 0 }
