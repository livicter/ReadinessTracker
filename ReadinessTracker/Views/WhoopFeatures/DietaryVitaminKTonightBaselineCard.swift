...
/// Elevates `nutrition.vitaminKMcg` (HK `dietaryVitaminK`) into Tonight | Baseline
/// on Today Body after Vitamin E. Net-new NutritionSummary + HK (#204).
/// Prefer dietaryVitaminK — clear leftover vitamin. Higher is better (soft 120 mcg).
struct DietaryVitaminKTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, vitaminKMcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if vitaminKMcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher vitamin K is generally favorable (soft ~120 mcg goal).
    ...
    private var deltaCaption: String {
        guard vitaminKMcg != nil else { return "No vitamin K logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms vitamin K, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [vitaminKColor.opacity(0.2), vitaminKColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [50, 84, 120]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryVitaminKBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.vitaminKMcg }
            .filter { $0 >= 0 }
