...
/// Elevates `nutrition.sugarGrams` (HK `dietarySugar`) into Tonight | Baseline
/// on Today Body after Dietary Fiber. Net-new NutritionSummary + HK (#176).
/// Prefer sugar — next unused dietary leftover after fiber. Lower is better.
struct DietarySugarTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, sugarGrams ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if sugarGrams == nil { return ("No reading", RTColor.secondaryText) }
    ...
    private var deltaCaption: String {
        guard sugarGrams != nil else { return "No sugar logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text("\(Int(tonight.rounded()))g")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(Int(tonight.rounded())) grams sugar, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [sugarColor.opacity(0.2), sugarColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DietarySugarBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.sugarGrams }
            .filter { $0 >= 0 }
