...
/// Elevates `nutrition.niacinMg` (HK `dietaryNiacin`) into Tonight | Baseline
/// on Today Body after Dietary Riboflavin. Net-new NutritionSummary + HK (#208).
/// Prefer dietaryNiacin — next leftover B-vitamin. Higher is better (soft 16 mg).
struct DietaryNiacinTonightBaselineCard: View {
    let niacinMg: Double?
    ...
    private var tonight: Double { max(0, niacinMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if niacinMg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher niacin is generally favorable (soft ~16 mg goal).
    ...
    private var deltaCaption: String {
        guard niacinMg != nil else { return "No dietary niacin logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMg(tonight)) milligrams dietary niacin, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [niacinColor.opacity(0.2), niacinColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [8, 11, 16]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryNiacinBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.niacinMg }
            .filter { $0 >= 0 }
