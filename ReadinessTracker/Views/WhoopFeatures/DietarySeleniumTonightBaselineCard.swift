...
/// Elevates `nutrition.seleniumMcg` (HK `dietarySelenium`) into Tonight | Baseline
/// on Today Body after Dietary Copper. Net-new NutritionSummary + HK (#212).
/// Prefer dietarySelenium — next leftover mineral. Higher is better (soft 55 mcg).
struct DietarySeleniumTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, seleniumMcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if seleniumMcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher selenium is generally favorable (soft ~55 mcg goal).
    ...
    private var deltaCaption: String {
        guard seleniumMcg != nil else { return "No dietary selenium logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms dietary selenium, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [seleniumColor.opacity(0.2), seleniumColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [20, 40, 55]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietarySeleniumBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.seleniumMcg }
            .filter { $0 >= 0 }
