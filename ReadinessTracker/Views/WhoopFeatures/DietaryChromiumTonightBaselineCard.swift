...
/// Elevates `nutrition.chromiumMcg` (HK `dietaryChromium`) into Tonight | Baseline
/// on Today Body after Dietary Phosphorus. Net-new NutritionSummary + HK (#216).
/// Prefer dietaryChromium — next leftover mineral (chromium). Higher is better (soft 35 mcg).
struct DietaryChromiumTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, chromiumMcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if chromiumMcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher chromium is generally favorable (soft ~35 mcg goal).
    ...
    private var deltaCaption: String {
        guard chromiumMcg != nil else { return "No dietary chromium logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms dietary chromium, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [chromiumColor.opacity(0.2), chromiumColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [15.0, 25.0, 35.0]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryChromiumBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.chromiumMcg }
            .filter { $0 >= 0 }
