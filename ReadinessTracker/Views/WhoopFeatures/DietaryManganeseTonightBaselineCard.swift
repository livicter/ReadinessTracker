...
/// Elevates `nutrition.manganeseMg` (HK `dietaryManganese`) into Tonight | Baseline
/// on Today Body after Dietary Selenium. Net-new NutritionSummary + HK (#213).
/// Prefer dietaryManganese — next leftover mineral. Higher is better (soft 2.3 mg).
struct DietaryManganeseTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, manganeseMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if manganeseMg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher manganese is generally favorable (soft ~2.3 mg goal).
    ...
    private var deltaCaption: String {
        guard manganeseMg != nil else { return "No dietary manganese logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMg(tonight)) milligrams dietary manganese, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(manganeseColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [manganeseColor.opacity(0.2), manganeseColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [0.8, 1.6, 2.3]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryManganeseBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.manganeseMg }
            .filter { $0 >= 0 }
