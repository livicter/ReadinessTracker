...
/// Elevates HealthKit `mindfulSession` (total duration minutes) into Tonight | Baseline on
/// Today body after Handwashing. Net-new DailyHealthData + HK (#189).
/// Prefer mindfulSession — unused category; bodyTemperature already feeds SkinTemperatureCard.
struct MindfulTonightBaselineCard: View {
    let minutes: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Mindful minutes vs soft ~10 min band + baseline.
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No mindful logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { formatMin($0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Mindful \(formatMin($0)) minutes, \(status.label)" }
                                ?? "No mindful logged"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [mindfulColor.opacity(0.2), mindfulColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
