...
struct DistanceTonightBaselineCard: View {
    ...
    private var status: (label: String, color: Color) {
        ...
        // Walk/run distance (km) — more volume vs personal baseline.
        if t >= 8 { return ("High", RTColor.optimal) }
        if t >= 5 { return ("Active", RTColor.good) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No walking distance logged" }
        let sign = delta >= 0 ? "+" : ""
    ...
    private var cardBody: some View {
        NativeCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                .accessibilityIdentifier(SurfaceID.distanceBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Walking Distance")
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [distanceColor.opacity(0.22), distanceColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DistanceBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.distanceWalkingRunningKm)
            .filter { $0 > 0 }
