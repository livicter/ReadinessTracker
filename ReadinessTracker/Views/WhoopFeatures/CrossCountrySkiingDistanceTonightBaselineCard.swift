...
/// Elevates HealthKit `distanceCrossCountrySkiing` (km) into Tonight | Baseline on
/// Today body after Skating Sports Distance. Net-new DailyHealthData + HK (#234).
/// Prefer distanceCrossCountrySkiing — next niche sports distance after skating.
struct CrossCountrySkiingDistanceTonightBaselineCard: View {
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No cross country skiing distance logged" }
        let sign = delta >= 0 ? "+" : "-"
        return String(format: "%@%.1f km vs baseline", sign, abs(delta))
    }

    private let xcSkiDistColor = Color(hex: "32ADE6")
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Cross Country Skiing Distance")
                    ...
                    Text(tonight.map { String(format: "%.1f", $0) } ?? "—")
                        ...
                        .accessibilityLabel(
                            tonight.map { "Rowing distance \(String(format: "%.1f", $0)) kilometers, \(status.label)" }
                                ?? "No cross country skiing distance"
                        )
                }
                ...
                .accessibilityIdentifier(SurfaceID.crossCountrySkiingDistanceBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Cross Country Skiing Distance")
                        ...
                        AnimatedSparkline(data: sparklineValues, color: xcSkiDistColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.crossCountrySkiingDistanceSpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(xcSkiDistColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [xcSkiDistColor.opacity(0.22), xcSkiDistColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum CrossCountrySkiingDistanceBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.distanceCrossCountrySkiingKm)
            .filter { $0 > 0 }
