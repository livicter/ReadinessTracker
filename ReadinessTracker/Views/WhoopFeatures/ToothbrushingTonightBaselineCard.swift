...
/// Elevates HealthKit `toothbrushingEvent` (total duration minutes) into Tonight | Baseline on
/// Today body after Basal Energy. Net-new DailyHealthData + HK (#185).
/// Prefer toothbrushing — hygiene-sparse category; duration clearer than event count.
struct ToothbrushingTonightBaselineCard: View {
    let minutes: Double?
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No brushing logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { formatMin($0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Toothbrushing \(formatMin($0)) minutes, \(status.label)" }
                                ?? "No brushing logged"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [brushColor.opacity(0.2), brushColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum ToothbrushingBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.toothbrushingMinutes)
            .filter { $0 > 0 }
