...
/// Elevates `nutrition.cholesterolMg` (HK `dietaryCholesterol`) into Tonight | Baseline
/// on Today Body after Dietary Potassium. Net-new NutritionSummary + HK (#192).
/// Prefer dietaryCholesterol — clear intake UX leftover after Na/K. Lower is better (soft 300 mg).
struct DietaryCholesterolTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, cholesterolMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if cholesterolMg == nil { return ("No reading", RTColor.secondaryText) }
    ...
    private var deltaCaption: String {
        guard cholesterolMg != nil else { return "No cholesterol logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text("\(Int(tonight.rounded()))")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(Int(tonight.rounded())) milligrams cholesterol, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(cholesterolColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [cholesterolColor.opacity(0.2), cholesterolColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DietaryCholesterolBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.cholesterolMg }
            .filter { $0 >= 0 }
