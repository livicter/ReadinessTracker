...
/// Elevates `nutrition.fatGrams` (HK `dietaryFatTotal`) into Tonight | Baseline
/// on Today Body after Carbohydrates. Net-new NutritionSummary + HK (#174).
/// Prefer total fat — next unused dietary leftover after carbs.
struct DietaryFatTonightBaselineCard: View {
    let fatGrams: Double?
    ...
    private var tonight: Double { max(0, fatGrams ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if fatGrams == nil { return ("No reading", RTColor.secondaryText) }
    ...
    private var deltaCaption: String {
        guard fatGrams != nil else { return "No fat logged" }
        let sign = delta >= 0 ? "+" : ""
        ...
        NativeCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if chartPoints.count >= 3 {
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [35, 70]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryFatBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.fatGrams }
            .filter { $0 >= 0 }
