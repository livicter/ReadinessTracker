...
/// Elevates HealthKit `forcedVitalCapacity` (L) into Tonight | Baseline on
/// Today body after Peak Expiratory Flow. Net-new DailyHealthData + HK (#225).
/// Prefer FVC — next lung-function leftover after PEF; soft glance bands (not a diagnosis).
struct ForcedVitalCapacityTonightBaselineCard: View {
    let liters: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        if t >= 4.5 { return ("Strong", RTColor.optimal) }
        if t >= 3.5 { return ("Solid", RTColor.good) }
        if t >= 2.5 { return ("Fair", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No FVC logged" }
        let sign = delta >= 0 ? "+" : "-"
        return String(format: "%@%.1f L vs baseline", sign, abs(delta))
    }

    private let fvcColor = Color(hex: "64D2FF")
    ...
    private var cardBody: some View {
        NativeCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                .accessibilityIdentifier(SurfaceID.forcedVitalCapacityBaselineCallout)
                ...
                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        AnimatedSparkline(data: sparklineValues, color: fvcColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.forcedVitalCapacitySpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [fvcColor.opacity(0.2), fvcColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = chartPoints.map(\.value) + [base, tonight ?? base, 2.5, 4.5].filter { $0 >= 0 }
        let lo = max(1.0, (vals.min() ?? 2.5) - 0.3)
        let hi = max(lo + 1.0, (vals.max() ?? 4.5) + 0.3)
        return lo...hi
    }
...
enum ForcedVitalCapacityBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.forcedVitalCapacityLiters)
            .filter { $0 > 0 }
