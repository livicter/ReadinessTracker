...
/// Elevates `nutrition.vitaminAMcg` (HK `dietaryVitaminA`) into Tonight | Baseline
/// on Today Body after Dietary Folate. Net-new NutritionSummary + HK (#202).
/// Prefer dietaryVitaminA — clear leftover vitamin. Higher is better (soft 900 mcg).
struct DietaryVitaminATonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, vitaminAMcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if vitaminAMcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher vitamin A is generally favorable (soft ~900 mcg goal).
    ...
    private var deltaCaption: String {
        guard vitaminAMcg != nil else { return "No vitamin A logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms vitamin A, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [vitaminAColor.opacity(0.2), vitaminAColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [400, 630, 900]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryVitaminABaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.vitaminAMcg }
            .filter { $0 >= 0 }
