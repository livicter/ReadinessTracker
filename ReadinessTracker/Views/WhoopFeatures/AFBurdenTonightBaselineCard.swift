...
/// Elevates HealthKit `atrialFibrillationBurden` (%) into Tonight | Baseline on
/// Today vitals after HR Recovery. Net-new DailyHealthData + HK (#168).
/// Prefer AF burden — wireable cardio % vs perfusion / falls fallbacks.
struct AFBurdenTonightBaselineCard: View {
    let percent: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // AF burden % of time; lower is better.
        if t <= 0.5 { return ("Clear", RTColor.optimal) }
        if t <= 2.0 { return ("Low", RTColor.good) }
        if t <= 5.0 { return ("Watch", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No AF burden" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { "\(String(format: "%.1f", $0))" } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "AF burden \(String(format: "%.1f", $0)) percent, \(status.label)" }
                                ?? "No AF burden"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [afColor.opacity(0.2), afColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        ...
        let hi = max(lo + 2.0, (vals.max() ?? 5) + 0.5)
        return lo...hi
    }
...
enum AFBurdenBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.atrialFibrillationBurdenPercent)
            .filter { $0 > 0 }
