...
/// Tonight | Baseline dual on Today vitals after Time in Daylight.
/// Net-new DailyHealthData + HK wire (#138). Skin temp already dualled;
/// Watch sleepScore already on complications — daylight was strongest unused.
struct TimeInDaylightTonightBaselineCard: View {
    let minutes: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Outdoor daylight minutes — more supports circadian cueing.
        if t >= 120 { return ("Strong", RTColor.optimal) }
        if t >= 60 { return ("Good", RTColor.good) }
        if t >= 30 { return ("Modest", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No daylight" }
        let sign = delta >= 0 ? "+" : ""
    ...
    private var yDomain: ClosedRange<Double> {
        ...
        let lo = max(0, (vals.min() ?? 20) - 10)
        let hi = max(lo + 40, (vals.max() ?? 120) + 15)
        return lo...hi
    }
...
enum TimeInDaylightBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.timeInDaylightMinutes)
            .filter { $0 > 0 }
