...
/// Elevates HealthKit `cyclingPowerWatts` (meters) into Tonight | Baseline on
/// Today body after Underwater Depth. Net-new DailyHealthData + HK (#156).
/// Prefer cycling power over stroke count / underwater / cadence — clearest unused activity volume.
struct CyclingPowerTonightBaselineCard: View {
    let watts: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Typical endurance ride power (avg watts).
        if t >= 220 { return ("Strong", RTColor.optimal) }
        if t >= 170 { return ("Solid", RTColor.good) }
        if t >= 120 { return ("Easy", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No cycling power logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.0f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Cycling power \(Int($0.rounded())) watts, \(status.label)" }
                                ?? "No cycling power"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [powerColor.opacity(0.22), powerColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = chartPoints.map(\.value) + [base, tonight ?? base, 170].filter { $0 > 0 }
        let lo = max(50, (vals.min() ?? 120) - 20)
        let hi = max(lo + 60, (vals.max() ?? 250) + 20)
        return lo...hi
    }
