...
/// Elevates HealthKit `distanceRowing` (km) into Tonight | Baseline on
/// Today body after Swim Strokes. Net-new DailyHealthData + HK (#229).
/// Prefer distanceRowing — clearest unused niche sports distance leftover.
struct RowingDistanceTonightBaselineCard: View {
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No rowing distance logged" }
        let sign = delta >= 0 ? "+" : "-"
        return String(format: "%@%.1f km vs baseline", sign, abs(delta))
    }

    private let rowDistColor = Color(hex: "30B0C7")
    ...
    private var cardBody: some View {
        NativeCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                .accessibilityIdentifier(SurfaceID.rowingDistanceBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Rowing Distance")
                        ...
                        AnimatedSparkline(data: sparklineValues, color: rowDistColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.rowingDistanceSpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [rowDistColor.opacity(0.22), rowDistColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
