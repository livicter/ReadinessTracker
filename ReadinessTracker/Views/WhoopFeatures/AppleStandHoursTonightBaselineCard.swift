...
/// Elevates HealthKit `appleStandHour` (Activity Stand ring hours) into a
/// Tonight | Baseline dual on Today strain after Exercise Time.
/// Net-new DailyHealthData + HK wire (#144). Exercise/workout minutes already
/// dualled; stand hours were strongest unused Watch ring field beyond rings.
struct AppleStandHoursTonightBaselineCard: View {
    let hours: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Apple Stand Hours vs typical 12-hour Activity ring goal.
        if t >= 12 { return ("Met", RTColor.optimal) }
        if t >= 9 { return ("Solid", RTColor.good) }
        if t >= 6 { return ("Light", RTColor.caution) }
        return ("Low", RTColor.warning)
    }

    private var sparklineValues: [Double] {
        Array(history.sorted { $0.date < $1.date }.suffix(7).map(\.hours))
    }

    private var chartPoints: [(date: Date, value: Double)] {
        history.sorted { $0.date < $1.date }.map { (date: $0.date, value: $0.hours) }
    }

    private var deltaCaption: String {
        guard tonight != nil else { return "No stand hours logged" }
        let sign = delta >= 0 ? "+" : ""
        return String(format: "%@%.0f hr vs baseline", sign, delta)
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.0f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Stand hours \(Int($0.rounded())), \(status.label)" }
                                ?? "No stand hours"
                        )
                }
                ...
                .accessibilityIdentifier(SurfaceID.appleStandHoursBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Stand Hours")
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [standColor.opacity(0.22), standColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        ...
        let hi = max(lo + 4, min(14, (vals.max() ?? 12) + 1))
        return lo...hi
    }
