...
/// Elevates `nutrition.proteinGrams` into a Tonight | Baseline dual on Today Body
/// (near Hydration / Caffeine). NutritionSummaryCard protein well (#106) stays
/// strain-detail glance — not re-chromed. Soft 100 g goal matches coaching.
struct ProteinTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, proteinGrams ?? 0) }
    ...
    private var sparklineValues: [Double] {
        Array(history.sorted { $0.date < $1.date }.suffix(7).map(\.protein))
    }

    private var chartPoints: [(date: Date, value: Double)] {
        history.sorted { $0.date < $1.date }.map { (date: $0.date, value: $0.protein) }
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text("\(Int(tonight.rounded()))g")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(Int(tonight.rounded())) grams protein, \(status.label)")
                }
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            ...
                            Text(deltaCaption)
                                ...
                                .foregroundStyle(delta < -15 ? RTColor.caution : RTColor.optimal)
                                .monospacedDigit()
                        }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [proteinColor.opacity(0.2), proteinColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum ProteinBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.proteinGrams }
            .filter { $0 >= 0 }
