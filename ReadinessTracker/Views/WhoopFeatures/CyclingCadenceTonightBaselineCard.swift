...
/// Elevates HealthKit `cyclingCadenceRpm` (meters) into Tonight | Baseline on
/// Today body after Swim Strokes. Net-new DailyHealthData + HK (#154).
/// Prefer cycling cadence over stroke count / underwater / cadence — clearest unused activity volume.
struct CyclingCadenceTonightBaselineCard: View {
    let rpm: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Typical endurance cadence ~70–100 rpm.
        if t >= 90 { return ("High", RTColor.optimal) }
        if t >= 75 { return ("Solid", RTColor.good) }
        if t >= 60 { return ("Easy", RTColor.caution) }
        return ("Low", RTColor.warning)
    }

    private var sparklineValues: [Double] {
        Array(history.sorted { $0.date < $1.date }.suffix(7).map(\.rpm))
    }

    private var chartPoints: [(date: Date, value: Double)] {
        history.sorted { $0.date < $1.date }.map { (date: $0.date, value: $0.rpm) }
    }

    private var deltaCaption: String {
        guard tonight != nil else { return "No cycling cadence logged" }
        let sign = delta >= 0 ? "+" : ""
        return String(format: "%@%.0f rpm vs baseline", sign, delta)
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.0f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Cycling cadence \(Int($0.rounded())) rpm, \(status.label)" }
                                ?? "No cycling cadence"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [cadenceColor.opacity(0.22), cadenceColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = chartPoints.map(\.value) + [base, tonight ?? base, 75].filter { $0 > 0 }
        let lo = max(40, (vals.min() ?? 60) - 5)
        let hi = max(lo + 20, (vals.max() ?? 100) + 5)
        return lo...hi
    }
