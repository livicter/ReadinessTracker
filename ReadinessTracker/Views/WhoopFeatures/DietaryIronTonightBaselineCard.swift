...
/// Elevates `nutrition.ironMg` (HK `dietaryIron`) into Tonight | Baseline
/// on Today Body after Vitamin B12. Net-new NutritionSummary + HK (#197).
/// Prefer dietaryIron — clear micronutrient Today UX after vitamins. Higher is better (soft 18 mg).
struct DietaryIronTonightBaselineCard: View {
    let ironMg: Double?
    ...
    private var tonight: Double { max(0, ironMg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if ironMg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher iron is generally favorable (soft ~18 mg goal).
    ...
    private var deltaCaption: String {
        guard ironMg != nil else { return "No dietary iron logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMg(tonight)) milligrams dietary iron, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [ironColor.opacity(0.2), ironColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [5, 10, 18]) { value in
                            AxisGridLine()
                            AxisValueLabel {
