...
/// Elevates `nutrition.vitaminB6Mg` (HK `dietaryVitaminB6`) into Tonight | Baseline
/// on Today Body after Vitamin K. Net-new NutritionSummary + HK (#205).
/// Prefer dietaryVitaminB6 — clear leftover B-vitamin after K. Higher is better (soft 1.7 mg).
struct DietaryVitaminB6TonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, vitaminB6Mg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if vitaminB6Mg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher vitamin B6 is generally favorable (soft ~1.7 mg goal).
    ...
    private var deltaCaption: String {
        guard vitaminB6Mg != nil else { return "No vitamin B6 logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMg(tonight)) milligrams vitamin B6, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(vitaminB6Color)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [vitaminB6Color.opacity(0.2), vitaminB6Color.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [0.5, 1.1, 1.7]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryVitaminB6Baseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.vitaminB6Mg }
            .filter { $0 >= 0 }
