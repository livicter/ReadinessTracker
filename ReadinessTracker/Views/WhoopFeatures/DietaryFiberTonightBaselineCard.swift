...
/// Elevates `nutrition.fiberGrams` (HK `dietaryFiber`) into Tonight | Baseline
/// on Today Body after Dietary Fat. Net-new NutritionSummary + HK (#175).
/// Prefer fiber — next unused dietary leftover after fat.
struct DietaryFiberTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, fiberGrams ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if fiberGrams == nil { return ("No reading", RTColor.secondaryText) }
    ...
    private var deltaCaption: String {
        guard fiberGrams != nil else { return "No fiber logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text("\(Int(tonight.rounded()))g")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(Int(tonight.rounded())) grams fiber, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [fiberColor.opacity(0.2), fiberColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [15, 30]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryFiberBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.fiberGrams }
            .filter { $0 >= 0 }
