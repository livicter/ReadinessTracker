...
/// Elevates HealthKit `distancePaddleSports` (km) into Tonight | Baseline on
/// Today body after Rowing Speed. Net-new DailyHealthData + HK (#231).
/// Prefer distancePaddleSports — next niche sports distance after rowing stack.
struct PaddleSportsDistanceTonightBaselineCard: View {
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No paddle sports distance logged" }
        let sign = delta >= 0 ? "+" : "-"
        return String(format: "%@%.1f km vs baseline", sign, abs(delta))
    }

    private let paddleDistColor = Color(hex: "0A84FF")
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 12) {
                    dualColumn(
                        ...
                        unit: "km",
                        color: status.color,
                        caption: "Paddle volume",
                        icon: "figure.kayaking"
                    )
                ...
                .accessibilityIdentifier(SurfaceID.paddleSportsDistanceBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Paddle Sports Distance")
                        ...
                        AnimatedSparkline(data: sparklineValues, color: paddleDistColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.paddleSportsDistanceSpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(paddleDistColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [paddleDistColor.opacity(0.22), paddleDistColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum PaddleSportsDistanceBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.distancePaddleSportsKm)
            .filter { $0 > 0 }
