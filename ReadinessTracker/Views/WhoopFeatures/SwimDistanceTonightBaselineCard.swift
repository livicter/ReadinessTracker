...
/// Elevates HealthKit `distanceSwimming` (meters) into Tonight | Baseline on
/// Today body after Six-Minute Walk. Net-new DailyHealthData + HK (#152).
/// Prefer swim distance over stroke count / underwater / cadence — clearest unused activity volume.
struct SwimDistanceTonightBaselineCard: View {
    let meters: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        if t >= 1500 { return ("Strong", RTColor.optimal) }
        if t >= 800 { return ("Solid", RTColor.good) }
        if t >= 400 { return ("Light", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No swim distance logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.0f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { "Swim distance \(Int($0.rounded())) meters, \(status.label)" }
                                ?? "No swim distance"
                        )
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [swimColor.opacity(0.22), swimColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        let vals = chartPoints.map(\.value) + [base, tonight ?? base, 800].filter { $0 > 0 }
        let lo = max(0, (vals.min() ?? 200) - 50)
        let hi = max(lo + 200, (vals.max() ?? 1500) + 100)
        return lo...hi
    }
...
enum SwimDistanceBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.distanceSwimmingMeters)
            .filter { $0 > 0 }
