...
/// Elevates `nutrition.iodineMcg` (HK `dietaryIodine`) into Tonight | Baseline
/// on Today Body after Dietary Manganese. Net-new NutritionSummary + HK (#214).
/// Prefer dietaryIodine — next leftover mineral. Higher is better (soft 150 mcg).
struct DietaryIodineTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, iodineMcg ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if iodineMcg == nil { return ("No reading", RTColor.secondaryText) }
        // Higher iodine is generally favorable (soft ~150 mcg goal).
    ...
    private var deltaCaption: String {
        guard iodineMcg != nil else { return "No dietary iodine logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(formatMcg(tonight))
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel("\(formatMcg(tonight)) micrograms dietary iodine, \(status.label)")
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [iodineColor.opacity(0.2), iodineColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [60, 100, 150]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryIodineBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.iodineMcg }
            .filter { $0 >= 0 }
