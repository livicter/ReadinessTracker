...
/// Elevates HealthKit `bodyMass` (kg) into Tonight | Baseline on
/// Today body after Blood Glucose. Net-new DailyHealthData + HK (#181).
/// Prefer bodyMass — unused body-composition after vitals glucose.
struct BodyMassTonightBaselineCard: View {
    let kg: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Composition glance vs 7-day baseline — not a clinical judgment.
        if abs(delta) < 0.5 { return ("Steady", RTColor.optimal) }
        if delta > 0 { return ("Higher", RTColor.caution) }
        return ("Lower", RTColor.good)
    }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No mass logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    ...
                    Text(tonight.map { String(format: "%.1f", $0) } ?? "—")
                        ...
                        .clipShape(Capsule())
                        .accessibilityLabel(
                            tonight.map { String(format: "Body mass %.1f kilograms, %@", $0, status.label) }
                                ?? "No mass logged"
                        )
                }
                ...
                .accessibilityIdentifier(SurfaceID.bodyMassBaselineCallout)
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [massColor.opacity(0.2), massColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
    ...
    private var yDomain: ClosedRange<Double> {
        ...
        guard let lo0 = vals.min(), let hi0 = vals.max() else { return 60...80 }
        let pad = max(1.0, (hi0 - lo0) * 0.2)
