...
/// Elevates HealthKit `appleStandTime` (cumulative stand minutes) into a
/// Tonight | Baseline dual on Today strain after Stand Hours.
/// Net-new DailyHealthData + HK wire (#221). Distinct from appleStandHour
/// category counts — stand *time* was leftover Activity ring quantity.
struct AppleStandTimeTonightBaselineCard: View {
    let minutes: Double?
    ...
    private var status: (label: String, color: Color) {
        ...
        // Apple Stand Time vs soft ~90-min cumulative stand-time goal.
        if t >= 90 { return ("Met", RTColor.optimal) }
        if t >= 60 { return ("Solid", RTColor.good) }
        if t >= 30 { return ("Light", RTColor.caution) }
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No stand time logged" }
        let sign = delta >= 0 ? "+" : ""
    ...
    private var cardBody: some View {
        NativeCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                .accessibilityIdentifier(SurfaceID.appleStandTimeBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Stand Time")
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(standTimeColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [standTimeColor.opacity(0.22), standTimeColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum AppleStandTimeBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.appleStandTimeMinutes)
            .filter { $0 > 0 }
