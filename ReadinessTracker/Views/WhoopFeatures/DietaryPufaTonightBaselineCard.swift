...
/// Elevates `nutrition.polyunsaturatedFatGrams` (HK `dietaryFatPolyunsaturated`) into Tonight | Baseline
/// on Today Body after Dietary Monounsaturated Fat. Net-new NutritionSummary + HK (#220).
/// Prefer dietaryFatPolyunsaturated — leftover fat subtype after sat/total.
struct DietaryPufaTonightBaselineCard: View {
    ...
    private var tonight: Double { max(0, polyunsaturatedFatGrams ?? 0) }
    ...
    private var status: (label: String, color: Color) {
        if polyunsaturatedFatGrams == nil { return ("No reading", RTColor.secondaryText) }
    ...
    private var deltaCaption: String {
        guard polyunsaturatedFatGrams != nil else { return "No polyunsaturated fat logged" }
        let sign = delta >= 0 ? "+" : ""
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Dietary Polyunsaturated Fat")
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [pufaColor.opacity(0.2), pufaColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .chartXAxis(.hidden)
                    .chartYAxis {
                        AxisMarks(position: .leading, values: [8.0, 15.0]) { value in
                            AxisGridLine()
                            AxisValueLabel {
...
enum DietaryPufaBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap { $0.nutrition.polyunsaturatedFatGrams }
            .filter { $0 >= 0 }
