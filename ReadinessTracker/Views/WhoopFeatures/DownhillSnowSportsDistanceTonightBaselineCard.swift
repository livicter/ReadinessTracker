...
/// Elevates HealthKit `distanceDownhillSnowSports` (km) into Tonight | Baseline on
/// Today body after Cross Country Skiing Speed. Net-new DailyHealthData + HK (#236).
/// Prefer distanceDownhillSnowSports — last listed niche sports distance leftover.
struct DownhillSnowSportsDistanceTonightBaselineCard: View {
    ...
    private var deltaCaption: String {
        guard tonight != nil else { return "No downhill snow sports distance logged" }
        let sign = delta >= 0 ? "+" : "-"
        return String(format: "%@%.1f km vs baseline", sign, abs(delta))
    }

    private let downhillDistColor = Color(hex: "FF6482")
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Downhill Snow Sports Distance")
                            .font(.headline.weight(.semibold))
                            .foregroundStyle(RTColor.primaryText)

                        Text("Snow · \(status.label)")
                            .font(.subheadline)
                    ...
                    Text(tonight.map { String(format: "%.1f", $0) } ?? "—")
                        ...
                        .accessibilityLabel(
                            tonight.map { "Rowing distance \(String(format: "%.1f", $0)) kilometers, \(status.label)" }
                                ?? "No downhill snow sports distance"
                        )
                }
                ...
                .accessibilityIdentifier(SurfaceID.downhillSnowSportsDistanceBaselineCallout)

                if sparklineValues.count >= 2 {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text("7-Day Downhill Snow Sports Distance")
                        ...
                        AnimatedSparkline(data: sparklineValues, color: downhillDistColor)
                            .frame(height: 28)
                            .accessibilityIdentifier(SurfaceID.downhillSnowSportsDistanceSpark)
                    }
                }
                ...
                if chartPoints.count >= 3 {
                    Chart {
                        ForEach(Array(chartPoints.enumerated()), id: \.offset) { _, point in
                            ...
                            .foregroundStyle(downhillDistColor)
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [downhillDistColor.opacity(0.22), downhillDistColor.opacity(0)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
enum DownhillSnowSportsDistanceBaseline {
    static func average(
        ...
        let values = history
            ...
            .prefix(window)
            .compactMap(\.distanceDownhillSnowSportsKm)
            .filter { $0 > 0 }
