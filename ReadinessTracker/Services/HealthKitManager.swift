...
class HealthKitManager: ObservableObject {
    ...
    func requestAuthorization() async {
        ...
        if #available(iOS 18.0, *) {
            ...
            if let estimatedEffortType = HKObjectType.quantityType(forIdentifier: .estimatedWorkoutEffortScore) {
                typesToRead.insert(estimatedEffortType)
            }
            if let rowingDistanceType = HKObjectType.quantityType(forIdentifier: .distanceRowing) {
                typesToRead.insert(rowingDistanceType)
            }
            if let rowingSpeedType = HKObjectType.quantityType(forIdentifier: .rowingSpeed) {
                typesToRead.insert(rowingSpeedType)
            }
            if let paddleDistanceType = HKObjectType.quantityType(forIdentifier: .distancePaddleSports) {
                typesToRead.insert(paddleDistanceType)
            }
    ...
    func fetchTodayData() async {
        ...
        async let wash = fetchHandwashingMinutes(predicate: predicate)
        async let mindful = fetchMindfulMinutes(predicate: predicate)
        ...
        async let fat = fetchBodyFatPercentage(predicate: predicate)
        ...
        async let standTime = fetchAppleStandTime(predicate: predicate)
        ...
        async let rowingDistance = fetchDistanceRowing(predicate: predicate)
        ...
        async let estimatedWorkoutEffort = fetchEstimatedWorkoutEffortScore(predicate: predicate)
        ...
        let data = DailyHealthData(
            ...
            appleStandTimeMinutes: await standTime,
            ...
            distanceRowingKm: await rowingDistance,
            ...
            estimatedWorkoutEffortScore: await estimatedWorkoutEffort,
    ...
    func fetchHistoricalData(days: Int = 30) async {
        ...
        for dayOffset in (1...days).reversed() {
            ...
            let washValue = await wash
            let mindfulValue = await mindful
            ...
            let standTimeValue = await standTime
            ...
            let rowingDistanceValue = await rowingDistance
            ...
            let estimatedWorkoutEffortValue = await estimatedWorkoutEffort
            ...
            let data = DailyHealthData(
                ...
                appleStandTimeMinutes: standTimeValue,
                ...
                distanceRowingKm: rowingDistanceValue,
                ...
                estimatedWorkoutEffortScore: estimatedWorkoutEffortValue,
    ...
    /// Day cumulative insulin delivery (IU). Sparse medical — fixture seeds UI.
    private func fetchInsulinDelivery(predicate: NSPredicate) async -> Double? {
    ...
    private func fetchBodyMass(predicate: NSPredicate) async -> Double? {
        guard let type = HKQuantityType.quantityType(forIdentifier: .bodyMass) else { return nil }
        return await fetchMostRecentQuantity(type: type, predicate: predicate, unit: .gramUnit(with: .kilo))
    }


    /// Latest lean body mass (kg). Sparse composition — fixture seeds UI.
    private func fetchLeanBodyMass(predicate: NSPredicate) async -> Double? {
    ...
    /// Latest body fat percentage (0–100). Sparse composition — fixture seeds UI.
    private func fetchBodyFatPercentage(predicate: NSPredicate) async -> Double? {
        guard let type = HKQuantityType.quantityType(forIdentifier: .bodyFatPercentage) else { return nil }
        let unit = HKUnit.percent()
        let raw = await fetchMostRecentQuantity(type: type, predicate: predicate, unit: unit)
    ...
    /// Day cumulative Apple Stand Time (minutes stood). Sparse on Simulator — fixture seeds UI.
    private func fetchAppleStandTime(predicate: NSPredicate) async -> Double? {
    ...
    /// Day cumulative rowing distance in kilometers. Sparse niche sport — fixture seeds UI. iOS 18+.
    private func fetchDistanceRowing(predicate: NSPredicate) async -> Double? {
        guard #available(iOS 18.0, *) else { return nil }
        guard let type = HKQuantityType.quantityType(forIdentifier: .distanceRowing) else { return nil }
        guard let meters = await fetchSumQuantity(type: type, predicate: predicate, unit: .meter()) else { return nil }
        return meters / 1000.0
    }


    /// Day average rowing speed (m/s). Sparse niche sport — fixture seeds UI. iOS 18+.
    private func fetchRowingSpeed(predicate: NSPredicate) async -> Double? {
        guard #available(iOS 18.0, *) else { return nil }
        guard let type = HKQuantityType.quantityType(forIdentifier: .rowingSpeed) else { return nil }
        let unit = HKUnit.meter().unitDivided(by: .second())
        return await withCheckedContinuation { continuation in
            let query = HKStatisticsQuery(
                quantityType: type,
    ...
    /// Day cumulative paddle sports distance in kilometers. Sparse niche sport — fixture seeds UI. iOS 18+.
    private func fetchDistancePaddleSports(predicate: NSPredicate) async -> Double? {
        guard #available(iOS 18.0, *) else { return nil }
        guard let type = HKQuantityType.quantityType(forIdentifier: .distancePaddleSports) else { return nil }
        guard let meters = await fetchSumQuantity(type: type, predicate: predicate, unit: .meter()) else { return nil }
        return meters / 1000.0
    }


    /// Day average paddle sports speed (m/s). Sparse niche sport — fixture seeds UI. iOS 18+.
    private func fetchPaddleSportsSpeed(predicate: NSPredicate) async -> Double? {
        guard #available(iOS 18.0, *) else { return nil }
        guard let type = HKQuantityType.quantityType(forIdentifier: .paddleSportsSpeed) else { return nil }
        let unit = HKUnit.meter().unitDivided(by: .second())
        return await withCheckedContinuation { continuation in
            let query = HKStatisticsQuery(
                quantityType: type,
    ...
    /// Day average estimated workout effort score (0–10 appleEffortScore). Sparse strain — fixture seeds UI. iOS 18+.
    private func fetchEstimatedWorkoutEffortScore(predicate: NSPredicate) async -> Double? {
        guard #available(iOS 18.0, *) else { return nil }
        guard let type = HKQuantityType.quantityType(forIdentifier: .estimatedWorkoutEffortScore) else { return nil }
        let unit = HKUnit.appleEffortScore()
        return await withCheckedContinuation { continuation in
            let query = HKStatisticsQuery(
                quantityType: type,
    ...
    /// Day total handwashing duration (minutes) from HKCategory handwashingEvent samples.
    private func fetchHandwashingMinutes(predicate: NSPredicate) async -> Double? {
        guard let type = HKObjectType.categoryType(forIdentifier: .handwashingEvent) else { return nil }
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                sampleType: type,
                predicate: predicate,
    ...
    /// Day total mindful session duration (minutes) from HKCategory mindfulSession samples.
    ...
    private func fetchNutrition(predicate: NSPredicate) async -> NutritionSummary {
        ...
        let fiber = await fetchSumQuantity(
        ...
        let sodium = await fetchSumQuantity(
            type: HKQuantityType.quantityType(forIdentifier: .dietarySodium)!,
            predicate: predicate,
            unit: .gramUnit(with: .milli)
        ).map { $0 }

        let potassium = await fetchSumQuantity(
        ...
        let iron = await fetchSumQuantity(
            type: HKQuantityType.quantityType(forIdentifier: .dietaryIron)!,
            predicate: predicate,
            unit: .gramUnit(with: .milli)
        ).map { $0 }

        let calcium = await fetchSumQuantity(
        ...
        let pantothenic = await fetchSumQuantity(
            type: HKQuantityType.quantityType(forIdentifier: .dietaryPantothenicAcid)!,
            predicate: predicate,
            unit: .gramUnit(with: .milli)
        ).map { $0 }

        let biotin = await fetchSumQuantity(
        ...
        let chromium = await fetchSumQuantity(
            type: HKQuantityType.quantityType(forIdentifier: .dietaryChromium)!,
            predicate: predicate,
            unit: .gramUnit(with: .micro)
        ).map { $0 }

        let molybdenum = await fetchSumQuantity(
            type: HKQuantityType.quantityType(forIdentifier: .dietaryMolybdenum)!,
            predicate: predicate,
            unit: .gramUnit(with: .micro)
        ).map { $0 }

        let chloride = await fetchSumQuantity(
        ...
        let pufa = await fetchSumQuantity(
