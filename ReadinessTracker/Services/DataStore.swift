...
enum UIFixture {
    ...
    static func history() -> [DailyHealthData] {
        ...
        return (0..<14).map { offset in
            ...
            // Rowing distance km (Honest #229). Sparse niche sport — seed for UI.
            let distanceRowingKmValue: Double? = {
                if offset == 0 { return 8.4 }
                if offset % 5 == 0 { return nil }
                return 1.5 + Double((offset * 13) % 95) / 10.0 // 1.5…10.9
            }()
            // Rowing speed m/s (Honest #230). Sibling after distance — seed for UI.
            let rowingSpeedMpsValue: Double? = {
                if offset == 0 { return 2.9 }
                if offset % 5 == 0 { return nil }
                return 1.4 + Double((offset * 11) % 28) / 10.0 // 1.4…4.1
            }()
            // Paddle sports distance km (Honest #231). Sparse niche sport — seed for UI.
            let distancePaddleSportsKmValue: Double? = {
                if offset == 0 { return 7.2 }
                if offset % 5 == 0 { return nil }
                return 1.2 + Double((offset * 17) % 108) / 10.0 // 1.2…11.9
            }()
            // Paddle sports speed m/s (Honest #232). Sibling after distance — seed for UI.
            let paddleSportsSpeedMpsValue: Double? = {
                if offset == 0 { return 2.7 }
                if offset % 5 == 0 { return nil }
                return 1.3 + Double((offset * 13) % 30) / 10.0 // 1.3…4.2
            }()
            // Skating sports distance km (Honest #233). Sparse niche sport — seed for UI.
            let distanceSkatingSportsKmValue: Double? = {
                if offset == 0 { return 6.8 }
                if offset % 5 == 0 { return nil }
                return 1.0 + Double((offset * 19) % 110) / 10.0 // 1.0…11.9
            }()
            // Cross-country skiing distance km (Honest #234). Sparse niche sport — seed for UI.
            let distanceCrossCountrySkiingKmValue: Double? = {
                if offset == 0 { return 9.5 }
                if offset % 5 == 0 { return nil }
            ...
            // Downhill snow sports distance km (Honest #236). Sparse niche sport — seed for UI.
            let distanceDownhillSnowSportsKmValue: Double? = {
                if offset == 0 { return 12.4 }
                if offset % 5 == 0 { return nil }
                return 2.5 + Double((offset * 29) % 155) / 10.0 // 2.5…17.9
            }()
            ...
            // Workout effort score 0–10 (Honest #227). Sparse strain — seed for UI.
            let workoutEffortScoreValue: Double? = {
            ...
            return DailyHealthData(
                ...
                distanceRowingKm: distanceRowingKmValue,
                rowingSpeedMps: rowingSpeedMpsValue,
                distancePaddleSportsKm: distancePaddleSportsKmValue,
                ...
                distanceDownhillSnowSportsKm: distanceDownhillSnowSportsKmValue,
                ...
                nutrition: NutritionSummary(
                    ...
                    vitaminEMg: {
                        ...
                        if offset % 5 == 0 { return nil }
                        return 6 + Double((offset * 7) % 17) // 6…22
                    }(),
                    vitaminKMcg: {
                        if offset == 0 { return 130 }
                        if offset % 5 == 0 { return nil }
                        return 40 + Double((offset * 29) % 161) // 40…200
                    }(),
                    vitaminB6Mg: {
                        if offset == 0 { return 1.9 }
                        if offset % 5 == 0 { return nil }
                        return 0.6 + Double((offset * 11) % 23) / 10.0 // 0.6…2.8
                    }(),
                    thiaminMg: {
                        if offset == 0 { return 1.4 }
                        if offset % 5 == 0 { return nil }
                        return 0.4 + Double((offset * 9) % 19) / 10.0 // 0.4…2.2
                    }(),
                    riboflavinMg: {
                        if offset == 0 { return 1.5 }
                        if offset % 5 == 0 { return nil }
                        return 0.5 + Double((offset * 11) % 20) / 10.0 // 0.5…2.4
                    }(),
                    niacinMg: {
                        if offset == 0 { return 18 }
                        if offset % 5 == 0 { return nil }
                        return 8 + Double((offset * 13) % 21) // 8…28
                    }(),
                    pantothenicAcidMg: {
                        if offset == 0 { return 5.5 }
                        if offset % 5 == 0 { return nil }
                        return 2.0 + Double((offset * 7) % 61) / 10.0 // 2.0…8.0
                    }(),
                    biotinMcg: {
                        if offset == 0 { return 35 }
                        if offset % 5 == 0 { return nil }
                        return 12 + Double((offset * 17) % 44) // 12…55
                    }(),
                    copperMg: {
                        if offset == 0 { return 1.0 }
                        if offset % 5 == 0 { return nil }
                        return 0.4 + Double((offset * 7) % 13) / 10.0 // 0.4…1.6
                    }(),
                    seleniumMcg: {
                    ...
                    iodineMcg: {
                        if offset == 0 { return 160 }
                        if offset % 5 == 0 { return nil }
                        return 60 + Double((offset * 19) % 191) // 60…250
                    }(),
                    phosphorusMg: {
                        if offset == 0 { return 820 }
                        if offset % 5 == 0 { return nil }
                        return 400 + Double((offset * 37) % 701) // 400…1100
                    }(),
                    chromiumMcg: {
                        if offset == 0 { return 42 }
                        if offset % 5 == 0 { return nil }
                        return 12 + Double((offset * 7) % 49) // 12…60
                    }(),
                    molybdenumMcg: {
                        if offset == 0 { return 52 }
                        if offset % 5 == 0 { return nil }
                        return 15 + Double((offset * 11) % 66) // 15…80
                    }(),
                    chlorideMg: {
                        if offset == 0 { return 2450 }
                        if offset % 5 == 0 { return nil }
                        return 1200 + Double((offset * 53) % 2301) // 1200…3500
                    }(),
