...
| Watch sleep page (Hours | Efficiency + stage legend) | Shipped. Wrist sleep glance elevates Hours|Eff dual callout (snapshot `sleepHours` + `sleepEfficiency`; no invented Need) + clearer Deep/REM/Light stage legend; optional Sleep score when present; compact live **Updated …** from `WatchSnapshot.date` (hide if missing) | [verify-watch-sleep.png](.audit/verify-watch-sleep.png) |
...
Wrist sleep glance elevates toward WHOOP/iPhone clarity: Hours|Efficiency dual callout from existing `WatchSnapshot` fields (`sleepHours`, `sleepEfficiency`; Sleep score when present), stacked Deep/REM/Light bar, and a clearer capsule stage legend. No invented Need hours (snapshot has no sleepNeed).
