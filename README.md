...
| Home Screen widget (Gym / Work / Sleep) | Shipped. Small + medium + **large** + **extra large** use Fitness-style `CompactTripleRingsView`; large adds Gym/Work/Sleep rows + HRV/RHR/Sleep hours; extra large (iPad / StandBy) extends large with readiness cue + G/W/S metric tiles; **small** + medium/large/extra-large show Fitness-style live **Updated …** (`Text(..., style: .relative)`) from App Group `lastUpdate` (hide if missing; getSnapshot shares App Group load with timeline); medium/large/extra-large also show interactive **Check-in** / **Evening** / **Trends** (`Link` → `readinesstracker://checkin/{morning|evening}` + `readinesstracker://trends`) | [verify-home-widget.png](.audit/verify-home-widget.png) · [verify-home-widget-small-updated.png](.audit/verify-home-widget-small-updated.png) · [verify-home-widget-large.png](.audit/verify-home-widget-large.png) · [verify-home-widget-extra-large.png](.audit/verify-home-widget-extra-large.png) · [verify-home-widget-updated.png](.audit/verify-home-widget-updated.png) · [verify-home-widget-live-updated.png](.audit/verify-home-widget-live-updated.png) · [verify-home-widget-checkin.png](.audit/verify-home-widget-checkin.png) · [verify-home-widget-deeplinks.png](.audit/verify-home-widget-deeplinks.png) |
| Watch dashboard hero (Gym / Work / Sleep + HRV\|RHR) | Shipped. Concentric Activity rings + elevated HRV\|RHR dual callout, Ready/Recovery cue, and compact live **Updated …** from `WatchSnapshot.date` (hide if missing); sleep/strain glances share the same cue; `WatchSnapshot` carries gym/work/sleep + vitals from WatchConnectivity | [verify-watch-dashboard.png](.audit/verify-watch-dashboard.png) |
| Watch strain page (Recovery | Strain dual arcs + callout) | Shipped. WHOOP dual concentric arcs (Recovery inner / Strain outer) plus elevated Recovery % \| Strain /21 dual callout, day cue, and compact live **Updated …** from `WatchSnapshot.date` (hide if missing) via shared `StrainRecoveryDualArcGeometry` + `CompactStrainRecoveryWheel` | [verify-watch-strain.png](.audit/verify-watch-strain.png) |
...
| Watch Check-in (Morning / Evening) | Shipped. Wrist sheet elevates to Morning|Evening picker + feel stars + habit toggles; `WatchSessionManager.sendCheckIn` sends correct `timeOfDay` (Evening can include Workout); parity intent with iPhone Check-in tab | [verify-watch-checkin.png](.audit/verify-watch-checkin.png) |
| Watch sleep page (Hours | Efficiency + stage legend) | Shipped. Wrist sleep glance elevates Hours|Eff dual callout (snapshot `sleepHours` + `sleepEfficiency`; no invented Need) + clearer Deep/REM/Core stage legend; optional Sleep score when present; compact live **Updated …** from `WatchSnapshot.date` (hide if missing) | [verify-watch-sleep.png](.audit/verify-watch-sleep.png) |
...
**Elsewhere.** History with weekly report. Home screen widgets (small / medium / large / extra large) with live Updated on small+ and Fitness-style Check-in / Evening / Trends deep links on medium/large, Lock Screen circular + rectangular (Fitness-style Gym/Work/Sleep triple rings), Watch dashboard (Fitness-style Gym/Work/Sleep triple rings + live Updated from `WatchSnapshot.date`), and Watch strain (WHOOP Recovery|Strain dual arcs + Recovery %|Strain /21 callout + Updated), Watch Check-in (Morning|Evening picker + stars/habits via WatchConnectivity), plus Watch complications (Fitness-style Gym/Work/Sleep circular + rectangular rings via Watch Widgets extension).
...
Fitness-style Gym / Work / Sleep concentric rings on the Watch glance hero (parity with Today + Home widget), plus elevated HRV|RHR dual callout and a Ready/Recovery cue from existing `WatchSnapshot` fields (no invented vitals). Scores and vitals arrive via WatchConnectivity.

![Watch dashboard](.audit/verify-watch-dashboard.png)

### Watch strain

WHOOP dual concentric arcs on the Watch strain page (Recovery inner / Strain outer), matching Today `StrainRecoveryWheel` colors and labels, plus an elevated Recovery % | Strain /21 dual callout and day cue under the wheel (from existing `WatchSnapshot` fields only — no invented metrics). Shared `StrainRecoveryDualArcGeometry` + `CompactStrainRecoveryWheel` (Watch App target membership).

![Watch strain](.audit/verify-watch-strain.png)

### Watch Check-in

Wrist Check-in sheet elevates beyond morning-only: Morning|Evening picker, feel stars (Digital Crown), habit toggles (Alcohol / Late caffeine / Sick; Evening adds Workout), and Save sends the selected `timeOfDay` through `WatchSessionManager` → phone `WatchConnectivityManager` / `MetadataStore`.

![Watch Check-in](.audit/verify-watch-checkin.png)

### Watch sleep

Wrist sleep glance elevates toward WHOOP/iPhone clarity: Hours|Efficiency dual callout from existing `WatchSnapshot` fields (`sleepHours`, `sleepEfficiency`; Sleep score when present), stacked Deep/REM/Core bar, and a clearer capsule stage legend. No invented Need hours (snapshot has no sleepNeed).

![Watch sleep](.audit/verify-watch-sleep.png)
...
**Small** + medium / large / extra-large Home widgets surface Fitness-style live **Updated …** (`Text(..., style: .relative)`) from App Group `lastUpdate` (`WidgetDataExporter`); `getSnapshot` shares the App Group loader with `getTimeline`. Cue hides gracefully when the key is missing. Small keeps a compact 9pt line under Readiness so triple rings stay primary. Lock accessories stay uncluttered.

![Home Screen small Updated](.audit/verify-home-widget-small-updated.png)
...
47. ~~Watch `WatchCheckInView` was a thin morning-only stars+toggles sheet (no evening, no dedicated PNG, weaker than iPhone Check-in).~~ Closed — Morning|Evening picker (watchOS equivalent; no `.segmented`); keep feel stars + habit toggles (Evening adds Workout); `WatchSessionManager.sendCheckIn` sends correct `timeOfDay`; [verify-watch-checkin.png](.audit/verify-watch-checkin.png) from `Scripts/capture-watch-checkin.sh` (ImageRenderer of `WatchCheckInChrome`).
48. ~~Watch `WatchSleepView` had hours + stage bar + metric rows but no dedicated Status/App-surfaces PNG (dashboard/strain/check-in do).~~ Closed — elevated Hours|Eff dual callout + clearer Deep/REM/Core stage legend from existing `WatchSnapshot` fields only (no invented Need); [verify-watch-sleep.png](.audit/verify-watch-sleep.png) from `Scripts/capture-watch-sleep.sh` (ImageRenderer of `WatchSleepChrome`).
49. ~~Watch `WatchDashboardView` rings were elevated but vitals stayed plain HRV/RHR/Recovery metric rows (sleep page already uses Hours|Eff dual callout).~~ Closed — elevated HRV|RHR dual callout + Ready/Recovery cue from existing `WatchSnapshot` fields only; [verify-watch-dashboard.png](.audit/verify-watch-dashboard.png) refreshed via `Scripts/capture-watch-dashboard.sh` (ImageRenderer of `WatchDashboardChrome`).
50. ~~Watch `WatchStrainView` already had WHOOP dual Recovery|Strain arcs (#30) but secondary rows stayed thin vs sleep/dashboard dual callouts.~~ Closed — elevated Recovery % | Strain /21 dual callout under the wheel + day cue from `WatchSnapshot.date` (no invented metrics; dual arcs kept); [verify-watch-strain.png](.audit/verify-watch-strain.png) refreshed via `Scripts/capture-watch-strain.sh` (ImageRenderer of elevated `WatchStrainChrome`).
51. ~~Medium/large/XL Home widgets show live Updated (#41/#42) but `SmallWidgetView` still omitted freshness.~~ Closed — compact Fitness-style live **Updated …** (`Text(..., style: .relative)`, 9pt) on `SmallWidgetView` when App Group `lastUpdate` present (hide if missing; rings stay primary); [verify-home-widget-small-updated.png](.audit/verify-home-widget-small-updated.png) from `Scripts/capture-home-widget-small-updated.sh` (ImageRenderer of `HomeWidgetSmallChrome`) + refreshed [verify-home-widget.png](.audit/verify-home-widget.png).
52. ~~Home widgets show live Updated (#41/#42/#51) but Watch dashboard/sleep/strain glances lacked freshness from existing `WatchSnapshot.date`.~~ Closed — compact Fitness-style live **Updated …** (`Text(..., style: .relative)`) via `WatchGlanceUpdatedCue` on `WatchDashboardView` (+ sleep/strain) when `snapshot.date` present (hide if missing); [verify-watch-dashboard.png](.audit/verify-watch-dashboard.png) refreshed via `Scripts/capture-watch-dashboard.sh` (ImageRenderer of `WatchDashboardChrome`).
53. ~~Honest #52 shipped Updated on live Watch dashboard/sleep/strain views and refreshed dashboard audit PNG, but sleep/strain audit chrome PNGs still omitted Updated.~~ Closed — audit sync: `WatchSleepChrome` / `WatchStrainChrome` include the same compact live **Updated …** from `snapshotDate` (parity with `WatchGlanceUpdatedCue` / dashboard chrome); [verify-watch-sleep.png](.audit/verify-watch-sleep.png) + [verify-watch-strain.png](.audit/verify-watch-strain.png) refreshed via `Scripts/capture-watch-sleep.sh` / `Scripts/capture-watch-strain.sh` (ImageRenderer); tree-guard / capture-surfaces already list both PNGs.
54–60. Closed on main — Apple Today polish through recommendation dedupe (PRs #72–#78 / Honest #54–#60).
61. ~~Sleep stage chrome still WHOOP-flavored (icons in stacked bar, loud score capsule, user-visible "Light").~~ Closed — Apple Health Sleep Stages: Awake→REM→Core→Deep stacked bar without in-bar icons; under-bar labels with soft tint wells; quiet "Xh total" subline; no score capsule on `SleepStageBreakdown` (score stays on Sleep Analysis); rename Light→Core across sleep UI labels (`lightSleepPercent` fields unchanged). Proof: [verify-sleep-stages.png](.audit/verify-sleep-stages.png) via `testSleepStagesSurface`.
