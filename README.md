...
| Watch strain page (Recovery | Strain dual arcs + callout) | Shipped. WHOOP dual concentric arcs (Recovery inner / Strain outer) plus elevated Recovery % \| Strain /21 dual callout and day cue from `WatchSnapshot` via shared `StrainRecoveryDualArcGeometry` + `CompactStrainRecoveryWheel` | [verify-watch-strain.png](.audit/verify-watch-strain.png) |
...
| Watch sleep page (Hours | Efficiency + stage legend) | Shipped. Wrist sleep glance elevates Hours|Eff dual callout (snapshot `sleepHours` + `sleepEfficiency`; no invented Need) + clearer Deep/REM/Light stage legend; optional Sleep score when present | [verify-watch-sleep.png](.audit/verify-watch-sleep.png) |
